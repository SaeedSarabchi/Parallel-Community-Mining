...
private double[] evaluateLMetrics(ArrayList<Map.Entry<Integer,Integer>> communityGraph, GlobalVariables gv) 
{
	...
	public void findLCommunitySeqSpark( int startNode, String inDir, String outDir, int threshold )
	{
		...
		for( int i = 0; i < threshold; i++ )
		{
			...
			// Find local community for a particular start point
			findLocalCommunitySpark( startNode, inDir, gv, allOutDegrees, visitedRecords );
			
			//If No Community Found, Make the visited feature of startNode false.
			if(gv.commPoints.size()==0)
				visitedRecords.put(startNode, false);
			else
				sb.append( "Community Detected: " );
	...
	public void findLocalCommunitySpark( int startNode, String inDir, GlobalVariables gv, Map<Integer,Integer> allOutDegrees, Map<Integer,Boolean> visitedRecords )
	{
		gv.commPoints = new ArrayList<Integer>();
		gv.commPoints.add( startNode );
		...
		while( true )
		{
			...
			// Compute L_in and L_ex and LGain
			computeLSpark(inDir, gv, potentialNodes);
		...
		ArrayList<Map.Entry<Integer,Integer>> communityGraph =  new ArrayList<Map.Entry<Integer,Integer>> ();
		communityGraph = extractCommunityGraphSpark(inDir, gv);
	...
	private ArrayList<Entry<Integer, Integer>> extractCommunityGraphSpark(String inDir, GlobalVariables gv) {
	...
	public void computeLSpark( String inDir, GlobalVariables gv, Map<Integer,String> potentialNodes )
	{
		// FileHandling
		HashMap<Integer, Integer> inDegrees =  new HashMap<Integer, Integer>();
		HashMap<Integer, Integer> outDegrees =  new HashMap<Integer, Integer>();
